...
// ── MenuBar ───────────────────────────────────────────────────────────────────
export function MenuBar({ onNew, onSaveAs, onFreeTransform, onWarp, onOpenFile, onPlaceEmbedded, onLoadFilesIntoStack, onClose }: MenuBarProps) {
  ...
  const menus: Record<string, MI[]> = {
    File: [
      ...
      sep,
      act('Close', onClose, '⌘W', useEditorStore.getState().layers.length === 0),
      act('Close All', onClose, '⌘⌥W', useEditorStore.getState().layers.length === 0),
      sep,
      act('Save', () => {
    ...
    Image: [
      ...
      sub('Image Rotation',
        ...
        act('Arbitrary…', () => useEditorStore.getState().openArbitraryRotationDialog()),
        sep,
    ...
    Select: [
      ...
      act('Focus Area…', () => useEditorStore.getState().openFocusAreaDialog()),
      sep,
    ...
    Filter: [
      ...
      sub('Pixelate',
        act('Mosaic…', () => openFilter('pixelate-mosaic', { cellSize: 16 })),
      ),
      sub('Distort',
    ...
    Window: [
      ...
      chk('Shapes', () => useEditorStore.getState().panelVisibility.shapes, () => useEditorStore.getState().togglePanelVisibility('shapes')),
